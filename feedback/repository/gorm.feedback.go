...
// Find is a method that finds a certain feedback from the database using an feedback id,
// also Find() uses only id as a key for selection
func (repo *FeedbackRepository) Find(id string) (*entity.Feedback, error) {

	feedback := new(entity.Feedback)
	err := repo.conn.Model(feedback).Where("id = ?", id).First(feedback).Error
...
// FindMultiple is a method that finds multiple feedbacks from the database the matches the given clientID
// In FindMultiple() only client_id is used as a key
func (repo *FeedbackRepository) FindMultiple(clientID string) []*entity.Feedback {

	var feedbacks []*entity.Feedback
	err := repo.conn.Model(entity.Feedback{}).Where("client_id = ?", clientID).Find(&feedbacks).Error
...
func (repo *FeedbackRepository) FindAll(seenStatus, pageNum int64) ([]*entity.Feedback, int64) {
	...
	switch seenStatus {
	case 0:
		repo.conn.Raw("SELECT * FROM feedbacks WHERE seen = ? ORDER BY created_at DESC LIMIT ?, 40",
			false, pageNum*40).Scan(&feedbacks)
		repo.conn.Raw("SELECT COUNT(*) FROM feedbacks WHERE seen = ?", false).Count(&count)
	case 1:
		repo.conn.Raw("SELECT * FROM feedbacks WHERE seen = ? ORDER BY created_at DESC LIMIT ?, 40", true,
			pageNum*40).Scan(&feedbacks)
...
func (repo *FeedbackRepository) SearchWRegx(key string, seenStatus, pageNum int64, columns ...string) ([]*entity.Feedback, int64) {
	...
	case 0:
		...
		repo.conn.Raw("SELECT COUNT(*) FROM feedbacks WHERE ("+strings.Join(whereStmt, "||")+") && seen = ?",
			sqlValues...).Count(&count)

		sqlValues = append(sqlValues, pageNum*40)
		repo.conn.Raw("SELECT * FROM feedbacks WHERE "+strings.Join(whereStmt, "||")+
			" && seen = ? ORDER BY created_at DESC LIMIT ?, 40", sqlValues...).Scan(&feedbacks)

	case 1:

		sqlValues = append(sqlValues, true)
		repo.conn.Raw("SELECT COUNT(*) FROM feedbacks WHERE ("+strings.Join(whereStmt, "||")+
			") && seen = ?", sqlValues...).Count(&count)
	...
	default:
		...
		sqlValues = append(sqlValues, pageNum*40)
		repo.conn.Raw("SELECT * FROM feedbacks WHERE "+strings.Join(whereStmt, "||")+
			" ORDER BY created_at DESC LIMIT ?, 40", sqlValues...).Scan(&feedbacks)
...
// SetToNull is a method that updates a certain feedback client_id value to null in the database
func (repo *FeedbackRepository) SetToNull(clientID string) error {

	err := repo.conn.Exec("UPDATE feedbacks SET client_id = NULL WHERE client_id = ?", clientID).Error
	if err != nil {
		return err
	}
...
// Delete is a method that deletes a certain feedback from the database using an feedback id.
// In Delete() id is only used as an key
func (repo *FeedbackRepository) Delete(id string) (*entity.Feedback, error) {
	feedback := new(entity.Feedback)
	err := repo.conn.Model(feedback).Where("id = ?", id).First(feedback).Error
...
// DeleteMultiple is a method that deletes a set of feedbacks from the database using an clientID.
// In Delete() client_id is only used as an key
func (repo *FeedbackRepository) DeleteMultiple(clientID string) []*entity.Feedback {
	var feedbacks []*entity.Feedback
	repo.conn.Model(feedbacks).Where("client_id = ?", clientID).Find(&feedbacks)
